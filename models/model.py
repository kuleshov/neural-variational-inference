...
class Model(object):
    ...
    def __init__(
        ...
        # create input vars
        (X, Y, idx1, idx2, S) = self.create_inputs()
        ...
        # create updates
        self.alpha = T.scalar(dtype=theano.config.floatX)  # learning rate
        self.updates = self.create_updates(
            grads, params, self.alpha, opt_alg, opt_params
        )

        # create givens
        my_givens = self.create_givens()

        # create methods for training / prediction
        mode = None
        # mode = NanGuardMode(nan_is_error=True, inf_is_error=True, big_is_error=False)
        self.train = theano.function(
            [idx1, idx2, self.alpha], [loss, acc], updates=self.updates,
            givens=my_givens, on_unused_input='warn', mode=mode
        )
        self.train_batch = theano.function(
            [X, Y, self.alpha], [loss, acc], updates=self.updates,
            on_unused_input='warn', mode=mode
        )
    ...
    def create_givens(self):
        (X, Y, idx1, idx2, S) = self.inputs
        return {X: self.train_set_x[idx1:idx2], Y: self.train_set_y[idx1:idx2]}
